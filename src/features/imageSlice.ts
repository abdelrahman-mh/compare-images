...
export const setFiles = ({ files, side }: { files: File[] | null; side: 'left' | 'right' }): AppThunk => {
  return async (dispatch) => {
    if (!files) {
      dispatch(setNotification({ message: 'Invalid image selected', type: 'error' }))
      return
    }
    ...
    for (let i = 0; i < filesToProcess; i++) {
      const file = files[i]
      const assetsSide = i % 2 === 0 ? side : side === 'left' ? 'right' : 'left'
